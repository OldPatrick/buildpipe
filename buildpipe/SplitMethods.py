...
class SlidingWindowSplit:
    """ A class to provide a SlidingWindowSplitter for the sklearn Pipeline
            ...
            n_folds (int):          amount of folds, folds are splits + 1
            window_step (int):      size of training window
            sliding_step (int):     size of sliding step for the next training window to begin
            forecast_step (int):    size of test window

            Methods:
            --------
            split(X, y, groups)
                creates the splits and give them back one slice after another (yield return)

            get_n_splits(X, y, groups):
                sklearn pipe needs the exact amount of splits, before they are calculated in the split funtion.
                Thus, this function catches it, over the initialization of a pipe object like GridSearchCV.
                If return of get_n_splits not matches splits in function split, This throws an error.
        """
    ...
    def __init__(
        ...
        if 0 in {self.window_step, self.forecast_step, self.sliding_step, self.n_splits}:
            raise ValueError(f"No zero values allowed for: "
    ...
    def split(self, X, y=None, groups=None):
        """ Generate indices to split data into training and test set.
            ...
            Args:
            -----
            X (array-like of shape (n_samples, n_features)):    Training data, where n_samples is the number of samples
        ...
        for window in windows_starts:
            ...
            if first_window:
                first_window = False
                flexible_train_size = windows_starts.step * self.window_step + (init_window_len - windows_starts.step)
                #varying train sizes for 1st window/split if window size would be uneven (equals the sum of n sliding windows rows
                #to the sum of the n expanding windows rows
                splits_total += 1
                print("First Split")
                yield (indices[:flexible_train_size],
                       indices[flexible_train_size:flexible_test_size + (init_window_len - windows_starts.step)])
            else:
                print("Next Split")
                flexible_train_size = windows_starts.step * self.window_step
                splits_total += 1
                yield (
                    indices[((window - windows_starts.step) * self.sliding_step):
                            (flexible_train_size + ((window - windows_starts.step) * self.sliding_step))],

                    indices[(flexible_train_size + ((window - windows_starts.step) * self.sliding_step)):
                            flexible_test_size + ((window - windows_starts.step) * self.sliding_step)]
                )
