...
class GCPDataset:
    ...
    def load_data(self, dataset: str, table: str) -> pd.DataFrame:
        ...
        Notes:
        ------
            - future versions should incorporate a flexible query, so that
              not all of the data has to be loaded, not only a default load all
            - function atm not more served as the working code will be developed
              with test datasets (boston, sklearn), (airline, sktime)
        """
