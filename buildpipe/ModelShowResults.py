...
# class needs a lot of refactoring; but I have ignored this class for a long time.
...
class ShowResults:
    ...
    def __init__(
        ...
        """ A constructor to forward the fitted model, the data and information about the splits and subestimators """
        self.model = model
    ...
    def show_model_results(self, y_pred, **perm_parameters):
        ...
        if self.model_is_trend_season == "yes" and self.model_not_combined:
            X_train_cut = X_train_cut.iloc[:, self.time_columns_start:]
            X_test_cut = X_test_cut.iloc[:, self.time_columns_start:]
        ...
        try:
            print(f"Best trained parameters from the pipeline (fit):", self.model.best_params_)
            print(f"Best trained mean score (R2, RMSE, ..) from the n-folds of the pipeline (fit):",self.model.best_score_)
            #"best_score_ = Mean cross-validated score of the best_estimator"
            try:
                rank_params = pd.DataFrame(self.model.cv_results_)
                display(rank_params.sort_values('rank_test_score', ascending=True))
            except AttributeError:
                print("No cv results function for this SearchMethod available, will continue without")
            finally:
                print("preparing permutation importance plots ...")
                perm_train = permutation_importance(self.model,
                                                    X_train_cut,
                                                    self.y_train,
                                                    n_repeats=30,
                                                    random_state=42,
                ...
                fig, ax = plt.subplots()
                ax.boxplot(perm_train.importances[sorted_idx_train].T,
                           vert=False, labels=X_train_cut.columns[sorted_idx_train])
                ax.set_title("Permutation Importances (train set)")
                plt.show()

                perm_test = permutation_importance(self.model,
                                                   X_test_cut,
                                                   self.y_test,
                                                   n_repeats=30,
                                                   random_state=42)
                ...
                ax2.boxplot(perm_test.importances[sorted_idx_test].T,
                            vert=False, labels=X_test_cut.columns[sorted_idx_test])
                ax2.set_title("Permutation Importances (test set)")
                plt.show()

        except BrokenPipeError:
            raise BrokenPipeError("No Pipeline fitted with baselines, continuing with remaining Fit values")

        finally:
            absolute_errors = np.abs(self.y_test - y_pred)
            scaled_error = np.mean(absolute_errors / mean_absolute_error(self.y_test, y_pred))

            print(f"Best predicted R2 score (predict):", r2_score(self.y_test, y_pred))
            print(f"Best predicted MAPE score (predict):", mean_absolute_percentage_error(self.y_test, y_pred))
            print(f"Best predicted MdAPE score (predict):", median_absolute_error(self.y_test, y_pred))
            print(f"Best predicted MSE score (predict):", mean_squared_error(self.y_test, y_pred))

            if type(scaled_error) is pd.Series:
                print("No MASE available for NaN Series")
            else:
                print(f"Best predicted MASE score (predict):", scaled_error)
